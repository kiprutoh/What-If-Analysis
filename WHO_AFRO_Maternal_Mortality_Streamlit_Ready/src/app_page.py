    ...
    if section == "Country scenario":
        ...
        with col_side:
            ...
            with st.container(border=True):
                st.markdown("#### Status quo baseline (latest data)")
                st.caption(
                    "Reset all levers to the country's **current status quo** before building a scenario. "
                    "At status quo, predicted MMR equals observed MMR."
                )
                sq1, sq2 = st.columns([2, 1])
                with sq1:
                    st.markdown(
                        f"**{country}** · Year **{data_year}** · Observed MMR **{observed_mmr:.0f}** "
                        f"· Skilled birth attendance **{baseline.get('Skilled Birth Attendance', 0):.0f}%** "
                        f"· ANC4 **{baseline.get('ANC4 Coverage', 0):.0f}%** · "
                        f"Gov health spend **{baseline.get('Gov Health Expenditure (% GDP)', 0):.2f}%** GDP"
                    )
                with sq2:
                ...
                st.session_state.status_quo_confirmed = st.checkbox(
                    "Status quo baseline reviewed — proceed with scenario adjustments",
                    value=st.session_state.get("status_quo_confirmed", True),
                )

            st.markdown(
                "**Green 🟢** = moving **up** lowers MMR (ANC4, gov spending, SBA). "
                "**Orange 🟠** = moving **up** raises MMR (risk factors)."
            )
