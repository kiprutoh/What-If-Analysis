...
def is_protective_indicator(column: str) -> bool:
    """True when a higher raw slider value is expected to lower MMR."""
    return float(FEATURE_TO_RISK_MULTIPLIER.get(column, 1.0)) < 0


def indicator_direction_hint(column: str) -> str:
    """Short UI hint for scenario sliders."""
    if is_protective_indicator(column):
        return "↑ on slider → lower MMR (more coverage, spending, or access)"
    return "↑ on slider → higher MMR (more risk or burden)"
